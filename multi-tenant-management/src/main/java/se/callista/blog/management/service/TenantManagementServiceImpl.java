...
public class TenantManagementServiceImpl implements TenantManagementService {
    ...
    private void createDatabase(String db, String password) {
        jdbcTemplate.execute((StatementCallback<Boolean>) stmt -> stmt.execute("CREATE DATABASE " + db + " CHARACTER SET utf8;"));
        jdbcTemplate.execute((StatementCallback<Boolean>) stmt -> stmt.execute("CREATE USER '" + db + "' IDENTIFIED BY '" + password + "';"));
        jdbcTemplate.execute((StatementCallback<Boolean>) stmt -> stmt.execute("CREATE USER '" + db + "'@'localhost' IDENTIFIED BY '" + password + "';"));
        jdbcTemplate.execute((StatementCallback<Boolean>) stmt -> stmt.execute("GRANT ALL PRIVILEGES ON " + db +".* TO '" + db + "';"));
